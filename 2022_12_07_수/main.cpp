...
void myInitTexture() 
{
	...
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGB,
		image.width, image.height, 0, GL_BGR, GL_UNSIGNED_BYTE,
		image.data);
	image.destroy();
...
void display()
{
	...
	//GLuint uTexCube = glGetUniformLocation(phong_prog, "uTexCube");
	GLuint uEPos = glGetUniformLocation(phong_prog, "uEPos");
	glUniform1i(uTexCube, 0);
	glUniform4f(uEPos, ePos[0], ePos[1], ePos[2], ePos[3]);
